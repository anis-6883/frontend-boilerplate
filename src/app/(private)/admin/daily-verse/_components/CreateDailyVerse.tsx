...
} from "@/features/daily-verse/dailyVerseApi";
...
export default function CreateDailyVerse({ edit, id }: { edit?: boolean; id?: string }) {
  const [createDailyVerse] = useCreateDailyVerseMutation();
  const [updateDailyVerse] = useUpdateDailyVerseMutation();
  const { data } = useGetSingleDailyVerseQuery(id, { skip: !edit });
  const router = useRouter();

  const handleSubmit = (values: any) => {
    if (edit) {
      updateDailyVerse({ id, data: values })
        .then((res: any) => {
          if (res.data.status) router.push(routes.admin.dailyManna.home);
        })
        .catch((err: any) => console.log(err));
    } else {
      createDailyVerse(values)
        .then((res: any) => {
          if (res.data.status) router.push(routes.admin.dailyManna.home);
        })
        .catch((err: any) => console.log(err));
