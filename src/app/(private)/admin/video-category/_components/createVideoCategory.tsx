...
import { vedioCategoryFormValidation } from "@/config/validation";
import {
  useCreateVideoCategoryMutation,
  useGetSinglevideoCategoryQuery,
  useUpdateVideoCategoryMutation
} from "@/features/video/videoCategoryApi";
...
export default function CreateContent({ edit, id }: { edit?: boolean; id?: string }) {
  ...
  const { data } = useGetSinglevideoCategoryQuery(id, { skip: !edit });
  ...
  const handleSubmit = (values: any) => {
    ...
    if (edit) {
      updateVideoCategory({ id, data: values })
        .then((res: any) => {
          if (res.data.status) router.push(routes.admin.videoCategory.home);
        })
  ...
  return (
    <Formik initialValues={initialValues} validationSchema={vedioCategoryFormValidation} onSubmit={handleSubmit} enableReinitialize>
