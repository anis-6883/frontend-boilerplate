...
import { useCreateLanguageMutation, useGetSingleLanguageQuery, useUpdateLanguageMutation } from "@/features/language/languageApi";
import { Field, Form, Formik } from "formik";
import { useRouter } from "next/navigation";
import { Button, Input } from "rizzui";

export default function CreateLanguage({ edit, id }: { edit?: boolean; id?: string }) {
  const [createLanguage] = useCreateLanguageMutation();
  const [updateLanguage] = useUpdateLanguageMutation();
  const { data } = useGetSingleLanguageQuery(id, { skip: !edit });
  ...
  const handleSubmit = (values: any) => {
    if (edit) {
      updateLanguage({ id, data: values })
        .then((res: any) => {
          if (res.data.status) router.push(routes.admin.language.home);
        })
